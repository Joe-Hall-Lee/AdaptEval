...
def calculate_curve_data(gpt4_file, judge_file, confidence_file, dataset_type, data_path):
    ...
    thresholds_to_test = np.linspace(min_conf, max_conf, num=100)
    accuracies = []
...
def plot_curves(pandalm_data, judgelm_data):
    ...
    # 绘制 PandaLM 曲线
    plt.plot(p_thresholds, p_accuracies,
             color='black', linestyle='-', linewidth=2, label='PandaLM 测试集')

    # 绘制 JudgeLM 曲线
    plt.plot(j_thresholds, j_accuracies,
    ...
    output_filename = "images/AdaptEval_performance_curves.png"
